...
const OtpInput = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Debug: Log the OTP comparison details
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-100">
      ...
      {process.env.NODE_ENV === 'development' && (
        <div className="fixed top-4 left-4 bg-yellow-100 p-4 rounded-lg border text-xs max-w-xs">
          <h3 className="font-bold mb-2">Debug Info:</h3>
          <p>Redux OTP: {reduxFormData.otp}</p>
          <p>Redux Email: {reduxFormData.email}</p>
          <p>Redux Username: {reduxFormData.username}</p>
          <p>LocalStorage: {localStorage.getItem('otpData') ? 'Available' : 'Not found'}</p>
